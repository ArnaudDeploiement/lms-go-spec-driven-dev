...
func (gq *GroupQuery) QueryCourse() *CourseQuery {
	query := (&CourseClient{config: gq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (gq *GroupQuery) QueryEnrollments() *EnrollmentQuery {
	query := (&EnrollmentClient{config: gq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (gq *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withCourse:       gq.withCourse.Clone(),
		withEnrollments:  gq.withEnrollments.Clone(),
...
func (gq *GroupQuery) WithCourse(opts ...func(*CourseQuery)) *GroupQuery {
	query := (&CourseClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (gq *GroupQuery) WithEnrollments(opts ...func(*EnrollmentQuery)) *GroupQuery {
	query := (&EnrollmentClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	gq.withEnrollments = query
	return gq
}
...
//
//	client.Group.Query().
//		GroupBy(group.FieldOrganizationID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Group.Query().
//		Select(group.FieldOrganizationID).
//		Scan(ctx, &v)
...
func (gq *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := gq.withCourse; query != nil {
		if err := gq.loadCourse(ctx, query, nodes, nil,
			func(n *Group, e *Course) { n.Edges.Course = e }); err != nil {
			return nil, err
		}
	}
	if query := gq.withEnrollments; query != nil {
		if err := gq.loadEnrollments(ctx, query, nodes,
			func(n *Group) { n.Edges.Enrollments = []*Enrollment{} },
			func(n *Group, e *Enrollment) { n.Edges.Enrollments = append(n.Edges.Enrollments, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (gq *GroupQuery) loadCourse(ctx context.Context, query *CourseQuery, nodes []*Group, init func(*Group), assign func(*Group, *Course)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Group)
	for i := range nodes {
...
func (gq *GroupQuery) loadEnrollments(ctx context.Context, query *EnrollmentQuery, nodes []*Group, init func(*Group), assign func(*Group, *Enrollment)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(enrollment.FieldGroupID)
	}
	query.Where(predicate.Enrollment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.EnrollmentsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (gq *GroupQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := gq.ctx.Fields; len(fields) > 0 {
		...
		if gq.withCourse != nil {
			_spec.Node.AddColumnOnce(group.FieldCourseID)
		}
	}
