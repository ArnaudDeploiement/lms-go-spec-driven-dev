...
func (cq *CourseQuery) QueryOrganization() *OrganizationQuery {
...
func (cq *CourseQuery) QueryModules() *ModuleQuery {
...
func (cq *CourseQuery) WithOrganization(opts ...func(*OrganizationQuery)) *CourseQuery {
	query := (&OrganizationClient{config: cq.config}).Query()
...
func (cq *CourseQuery) WithModules(opts ...func(*ModuleQuery)) *CourseQuery {
	query := (&ModuleClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//
//	client.Course.Query().
//		GroupBy(course.FieldOrganizationID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Course.Query().
//		Select(course.FieldOrganizationID).
//		Scan(ctx, &v)
...
func (cq *CourseQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Course, error) {
	...
	if query := cq.withOrganization; query != nil {
		if err := cq.loadOrganization(ctx, query, nodes, nil,
			func(n *Course, e *Organization) { n.Edges.Organization = e }); err != nil {
			return nil, err
		}
	}
...
func (cq *CourseQuery) loadOrganization(ctx context.Context, query *OrganizationQuery, nodes []*Course, init func(*Course), assign func(*Course, *Organization)) error {
...
func (cq *CourseQuery) loadModules(ctx context.Context, query *ModuleQuery, nodes []*Course, init func(*Course), assign func(*Course, *Module)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(module.FieldCourseID)
	}
	query.Where(predicate.Module(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(course.ModulesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (cq *CourseQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := cq.ctx.Fields; len(fields) > 0 {
		...
		if cq.withOrganization != nil {
			_spec.Node.AddColumnOnce(course.FieldOrganizationID)
		}
	}
