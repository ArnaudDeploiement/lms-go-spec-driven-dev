...
func (oq *OrganizationQuery) QueryContents() *ContentQuery {
	query := (&ContentClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oq *OrganizationQuery) QueryCourses() *CourseQuery {
...
func (oq *OrganizationQuery) WithContents(opts ...func(*ContentQuery)) *OrganizationQuery {
	query := (&ContentClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withContents = query
	return oq
}
...
func (oq *OrganizationQuery) WithCourses(opts ...func(*CourseQuery)) *OrganizationQuery {
	query := (&CourseClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (oq *OrganizationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Organization, error) {
	...
	if query := oq.withContents; query != nil {
		if err := oq.loadContents(ctx, query, nodes,
			func(n *Organization) { n.Edges.Contents = []*Content{} },
			func(n *Organization, e *Content) { n.Edges.Contents = append(n.Edges.Contents, e) }); err != nil {
			return nil, err
		}
	}
	if query := oq.withCourses; query != nil {
...
func (oq *OrganizationQuery) loadContents(ctx context.Context, query *ContentQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Content)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(content.FieldOrganizationID)
	}
	query.Where(predicate.Content(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(organization.ContentsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (oq *OrganizationQuery) loadCourses(ctx context.Context, query *CourseQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Course)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(course.FieldOrganizationID)
	}
