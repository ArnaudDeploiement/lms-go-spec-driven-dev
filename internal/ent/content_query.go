...
func (cq *ContentQuery) QueryOrganization() *OrganizationQuery {
...
func (cq *ContentQuery) WithOrganization(opts ...func(*OrganizationQuery)) *ContentQuery {
	query := (&OrganizationClient{config: cq.config}).Query()
...
//		GroupBy(content.FieldOrganizationID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Content.Query().
//		Select(content.FieldOrganizationID).
//		Scan(ctx, &v)
...
func (cq *ContentQuery) loadOrganization(ctx context.Context, query *OrganizationQuery, nodes []*Content, init func(*Content), assign func(*Content, *Organization)) error {
...
func (cq *ContentQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := cq.ctx.Fields; len(fields) > 0 {
		...
		if cq.withOrganization != nil {
			_spec.Node.AddColumnOnce(content.FieldOrganizationID)
		}
	}
