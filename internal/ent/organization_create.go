...
func (oc *OrganizationCreate) SetSettings(m map[string]interface{}) *OrganizationCreate {
	oc.mutation.SetSettings(m)
	return oc
}
...
func (oc *OrganizationCreate) AddContentIDs(ids ...uuid.UUID) *OrganizationCreate {
	oc.mutation.AddContentIDs(ids...)
	return oc
}

// AddContents adds the "contents" edges to the Content entity.
func (oc *OrganizationCreate) AddContents(c ...*Content) *OrganizationCreate {
	ids := make([]uuid.UUID, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
	return oc.AddContentIDs(ids...)
}

// AddCourseIDs adds the "courses" edge to the Course entity by IDs.
func (oc *OrganizationCreate) AddCourseIDs(ids ...uuid.UUID) *OrganizationCreate {
	oc.mutation.AddCourseIDs(ids...)
	return oc
}

// AddCourses adds the "courses" edges to the Course entity.
func (oc *OrganizationCreate) AddCourses(c ...*Course) *OrganizationCreate {
	ids := make([]uuid.UUID, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
	return oc.AddCourseIDs(ids...)
}
...
func (oc *OrganizationCreate) AddEnrollmentIDs(ids ...uuid.UUID) *OrganizationCreate {
	oc.mutation.AddEnrollmentIDs(ids...)
	return oc
}

// AddEnrollments adds the "enrollments" edges to the Enrollment entity.
func (oc *OrganizationCreate) AddEnrollments(e ...*Enrollment) *OrganizationCreate {
	ids := make([]uuid.UUID, len(e))
	for i := range e {
		ids[i] = e[i].ID
	}
	return oc.AddEnrollmentIDs(ids...)
}
...
func (oc *OrganizationCreate) defaults() {
	...
	if _, ok := oc.mutation.Settings(); !ok {
		v := organization.DefaultSettings
		oc.mutation.SetSettings(v)
	}
...
func (oc *OrganizationCreate) createSpec() (*Organization, *sqlgraph.CreateSpec) {
	...
	if value, ok := oc.mutation.Settings(); ok {
	...
	if nodes := oc.mutation.ContentsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
	...
	if nodes := oc.mutation.CoursesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
	...
	if nodes := oc.mutation.EnrollmentsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
